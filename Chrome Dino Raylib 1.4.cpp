// =====================================================================
// Chrome Dino Raylib v1.4  (raylib 6.0)
// 修复：
//  1. 下蹲碰撞盒按地面锚定 —— 修复全屏/窗口下蹲无敌：下蹲只能躲翼龙，
//     仙人掌仍会命中（还原原版行为，任意窗口缩放一致）。
//  2. 空中按 ↓ 立即取消上行速度 —— 修复起跳上行阶段按 ↓ 会被 3 倍速降
//     系数反向加速而跳得极高的 Bug（对应原版 setSpeedDrop: jumpVelocity=1）。
// =====================================================================
#include "raylib.h"
#include <vector>
#include <cstdlib>
...
const float DUCK_SPRITE_HEIGHT = 25.0f;  // 下蹲精灵原始高度（px），贴地锚定用
...
public:
	...
	Dinosaur(float groundY) : groundY(groundY)
	{
		...
		duckBoxes = { {1, 18, 55, 25} };  // 下蹲（原版像素盒，可躲开所有翼龙）
	}
	...
	void Update(float dt, float speed, bool duckHeld)
	{
		...
		if (isDucking)
		{
			...
			// v1.4 修复：下蹲精灵按原始高度贴地（底边对齐地面线），
			// 碰撞盒随地面锚定，全屏/窗口一致，仙人掌仍能命中
			rect.y = groundY - DUCK_SPRITE_HEIGHT * globalScale;
		}
		else
		{
	...
	void SpeedDrop(bool drop)
	{
		// 原版 setSpeedDrop()：空中按 ↓ 立即取消上行速度（jumpVelocity=1）。
		// 否则 3 倍速降系数会反向放大上行速度，让恐龙跳得极高（v1.4 修复）
		speedDrop = drop;
		if (drop && velocityY < 0)
			velocityY = 1.0f * YS * scaleY;
	}
	...
	void SnapToGround()
	{
		// v1.4：窗口缩放/全屏切换后重新贴地；下蹲按精灵实际高度锚定地面
		if (isDucking)
			rect.y = groundY - DUCK_SPRITE_HEIGHT * globalScale;
		else
			rect.y = groundY - normalHeight;
...
public:
	...
	// v1.3 修复：窗口缩放后按比例重新缩放（保持地面锚定，碰撞盒同步缩放）
	void Rescale(float ratio, float oldGroundY, float newGroundY)
	{
		x *= ratio;
		y = newGroundY - (oldGroundY - y) * ratio;
		width *= ratio;
		height *= ratio;
		gap *= ratio;
		for (auto& b : boxes)
...
{
public:
	float x, y, width, height, gap;   // 基准坐标单位（800x400），绘制时乘 globalScale

	Cloud()
	{
		width = 46.0f * XS;
		height = 14.0f * YS;
		y = (30.0f + (float)(rand() % 42)) * YS;   // 原版天空带 30..71
		x = (float)BASE_WIDTH;
		gap = (100.0f + (float)(rand() % 301)) * XS;   // 原版 100..400
	}
	...
	void Draw()
	{
		// v1.3 修复：尺寸/位置按当前窗口缩放实时计算，窗口大小变化时不再错位
		Color c = DN(LIGHTGRAY, {55, 55, 55, 255});
		float sx = x * globalScale;
		float sy = y * globalScale;
		float sw = width * globalScale;
		float sh = height * globalScale;
		float radius1 = sh * 0.5f;
		float radius2 = sh * 0.67f;
		DrawCircle(sx + sw * 0.33f, sy + sh * 0.5f, radius1, c);
		DrawCircle(sx + sw * 0.58f, sy + sh * 0.33f, radius2, c);
		DrawCircle(sx + sw * 0.83f, sy + sh * 0.5f, radius1, c);
	}
};
...
{
	// 原版：ceil(0.2/1000 * dt_ms * speed) px/帧（基准单位）
	float es = ceilf(0.2f / 1000.0f * dt * 1000.0f * speed) * XS;
	for (auto& c : clouds)
		c.Update(es, dt);
	...
	if (!clouds.empty() && clouds.size() < 6)
	{
		...
		if (currentWidth - last.x * globalScale > last.gap * globalScale && (rand() % 100) < 50)
...
int main()
{
	...
	currentWidth = BASE_WIDTH * 2;
	currentHeight = BASE_HEIGHT * 2;
	InitWindow(currentWidth, currentHeight, "Chrome Dino Game v1.4");
	...
	while (!WindowShouldClose())
	{
		...
		{
			float oldGlobalScale = globalScale;
			...
			// v1.3 修复：已生成的障碍物按比例同步缩放（云存基准单位，自动适配）
			if (oldGlobalScale > 0 && oldGlobalScale != globalScale)
			{
				float ratio = globalScale / oldGlobalScale;
				for (auto& obs : obstacles)
					obs.Rescale(ratio, oldGroundY, groundY);
			}
		}
		...
		if (gameStarted && !gameOver)
		{
			...
			if (dino.isJumping)
				dino.SpeedDrop(duckHeld);    // 原版：空中按住下键速降（x3，v1.4 修复上行反向加速）
			else
				dino.Duck(duckHeld);
